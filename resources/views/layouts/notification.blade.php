{{-- Thông báo --}}
<li class="nav-item topbar-icon dropdown hidden-caret list-unstyled ms-2">
    <a class="nav-link dropdown-toggle" href="#" id="notifDropdown" role="button" data-bs-toggle="dropdown"
        aria-haspopup="true" aria-expanded="false">
        <i class="fa fa-bell"></i>
        <span class="notification text-danger">{{ $notifications->where('is_read', 0)->count() }}</span>
    </a>
    <ul class="dropdown-menu notif-box animated fadeIn p-3" aria-labelledby="notifDropdown" style="width: 400px;">
        <li>
                    ...
                    @foreach ($notifications as $notification)
                        <a data-id="{{ $notification->notification_id }}"
                            ...
                            <div class="d-flex align-items-center w-100">
                                <div class="notif-icon notif-primary pe-2">
                                    <i class="fa fa-bell"></i>
                                </div>
                                <div class="notif-content">
                                    <span class="block text-truncate"
                                        style="max-width: 260px;">{{ $notification->title }}</span>
                                    <span class="time"
                                ...
                                <div class="notif-icon text-danger ms-auto">
                                    <form
                                        action="{{ route('notification/markAsDeleted', $notification->notification_id) }}"
                                        method="POST">
                                        @csrf @method('PUT')
        ...
        <li>
            <button class="see-all btn cursor-pointer p-0" id="showNotificationsBtn">Xem toàn bộ thông báo
                ...
                <div class="d-flex align-items-center w-100">
                    ...
                    <div>
                        <div class="fw-bold text-truncate" style="max-width: 260px;">
                            {{ $notification->title ?? 'Thông báo' }}</div>
                        <small class="text-muted">{{ $notification->created_at->diffForHumans() }}</small>
                    </div>
                    <div class="notif-icon text-danger ms-auto">
                        <form action="{{ route('notification/markAsDeleted', $notification->notification_id) }}"
                            method="POST">
                            @csrf @method('PUT')
                            <button class="btn btn-sm">
                                <i class="fa fa-trash"></i>
