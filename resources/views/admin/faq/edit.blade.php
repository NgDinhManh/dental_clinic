...
@section('content')
    <div class="page-inner">
        ...
        <form action="{{ route('admin/faq/update', $faq->faq_id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="question" class="form-label">Câu hỏi</label>
                <textarea type="text" class="form-control" id="question" name="question" rows="8" required> {{ $faq->question }}</textarea>
            </div>
            <div class="mb-3">
                <label for="answer" class="form-label">Câu trả lời</label>
                <textarea class="form-control" id="answer" name="answer" rows="8" required>{{ $faq->answer }}</textarea>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label for="faq_order" class="form-label">Thứ tự</label>
                <input type="number" class="form-control" id="faq_order" name="faq_order" value="{{ $faq->faq_order }}">
            </div>
            <div class="row">
